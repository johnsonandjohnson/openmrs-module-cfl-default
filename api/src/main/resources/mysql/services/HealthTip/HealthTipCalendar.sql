SELECT EXECUTION_DATE,
       ...
       HEALTH_TIP_ID
FROM (
         SELECT TIMESTAMP(selected_date,
                          times) AS EXECUTION_DATE,
                1                AS MESSAGE_ID,
                  ...
                  WHERE selected_date >= DATE(:startDateTime)
                    ...
                    AND (DAYOFMONTH(selected_date) < IF
                      (
                              ...
                              OR :Frequency_of_the_message = 'Daily',
                              32,
                              8
                      )
                      )
    ...
    FROM (
             SELECT all_ht.HT_ID as HT_ID
             FROM (
                      SELECT ROW_NUMBER() OVER cs_w AS 'rn',
                             concept_id             AS HT_ID
                          ...
                          WHERE mt.name = 'Health tip'
                            ...
                            AND mar.answered_time >
                                IF((
                                   ...
                                   '1900-01-01',
                                   IFNULL(
                                           (SELECT MAX(answered_time)
                                            FROM (SELECT question, source_id, answered_time
                                                  FROM messages_actor_response mar
                                                  WHERE source_type like 'SCHEDULED_SERVICE_GROUP') mar
                                                     JOIN messages_scheduled_service_group mssg
                                                          on mar.source_id = mssg.messages_scheduled_service_group_id
...
WHERE EXECUTION_DATE > GET_PREDICTION_START_DATE_FOR_HEALTH_TIP(:patientId,
                                                                :actorId,
